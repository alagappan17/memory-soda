...
| Page | Covers |
|---|---|
| [`MemorySodaClient`](./client.md) | `recall`, `prepareAndRecall`, `health`, `ping`, config |
| [`client.threads`](./threads.md) | thread lifecycle |
| [`client.workingMemory`](./working-memory.md) | messages, prepare, compact, stats |
| [`client.semantic`](./semantic-memory.md) | facts and entities |
| [Error handling](./errors.md) | error classes and retry patterns |
| [Type reference](./types.md) | every exported type |
...
**Nothing is retried automatically.** See [Error handling](./errors.md) for a
retry helper.
...
| | Why | Endpoint |
|---|---|---|
| `chat` | A demo endpoint that runs the model server-side. Use `prepare` + `recall` with your own model. | [`POST /v1/memory/working/threads/:id/chat`](../api/working-memory.md#chat) |
| Episodic CRUD | Admin surface. Normal reads go through `recall({ include: ['episodes'] })`. | [Episodic memory](../api/episodic-memory.md) |
| Projects, API keys, users | Session-authenticated dashboard routes, not integration surface. | [Dashboard routes](../api/dashboard.md) |

There is also **no `add()`** — facts cannot be written directly, only derived
from messages. See [Known limitations](../introduction/overview.md#known-limitations).

---

## Next

- [`MemorySodaClient`](./client.md)
