...
1. **Every fact is about the user.** Subject must be `"user"`. Enforced in code
   too — see [Semantic memory](./semantic-memory.md#every-fact-is-about-the-user).
...
retry cap) or orphaned. See [Background jobs](../operations/background-jobs.md).
...
The [Playground](../dashboard/playground.md) polls for new episodes and emits a
...
- [Semantic memory](./semantic-memory.md) — what the pipeline produces
- [The bi-temporal model](./bi-temporal-model.md) — how contradictions are recorded
- [Background jobs](../operations/background-jobs.md) — what drives it
