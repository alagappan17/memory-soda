...
Base path: `/v1/memory/semantic` · Auth: [API key](./authentication.md)

Unranked, chronological reads over the fact store, plus soft deletion. For
prompt-ready ranked output use [`POST /v1/memory/recall`](./recall.md).

SDK equivalent: [`client.semantic`](../sdk/semantic-memory.md)
...
> edited — see [Curating memory](../guides/curating-memory.md).
...
- [Recall API](./recall.md) — ranked, prompt-ready reads
