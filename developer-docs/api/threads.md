...
Base path: `/v1/threads` · Auth: [API key](./authentication.md)

SDK equivalent: [`client.threads`](../sdk/threads.md)
...
> No `messageCount` — use [`GET …/stats`](./working-memory.md#get-v1memoryworkingthreadsthreadidstats).
...
Queue [episode extraction](../concepts/episodic-memory.md) immediately.
...
| List threads | Dashboard only — [`GET /dashboard/threads`](./dashboard.md). |
...
- [Working memory API](./working-memory.md) — appending and reading messages
- [`client.threads`](../sdk/threads.md)
