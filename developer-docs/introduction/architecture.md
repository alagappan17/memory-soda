...
`/health` and `/auth/*` are public. See [Authentication](../api/authentication.md).

---

## Data model

Nine tables. Full DDL in [Database schema](../reference/database-schema.md).
...
error middleware yet — see [Errors](../reference/errors.md).
...
> API process. See [Background jobs](../operations/background-jobs.md).
...
remedy there is deletion; see [Curating memory](../guides/curating-memory.md).
