...
Every [episode](../concepts/episodic-memory.md) for the dataset — the summary,
...
The extracted [semantic memory](../concepts/semantic-memory.md), newest first.
...
[Curating memory](../guides/curating-memory.md).
...
Use it to check [entity resolution](../concepts/semantic-memory.md#resolution).
...
merge behaviour — see [Tuning retrieval](../guides/tuning-retrieval.md).
...
| | Alternative |
|---|---|
| Delete a whole dataset | No UI, no endpoint — [Privacy and data deletion](../operations/privacy-and-deletion.md) |
| Edit or add a fact | Delete only; facts are derived |
| See extraction failures | SQL, as above |
| See which facts a past reply used | [Playground](./playground.md), current session only |
...
[Semantic memory API](../api/semantic-memory.md).

---

## Next

- [Playground](./playground.md) — watch extraction happen live
