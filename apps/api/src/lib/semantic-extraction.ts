...
const buildGraphSystem = (minConfidence: number) => `You are a knowledge graph extraction system. Given an episode summary and key learnings, extract a structured knowledge graph.
...
export async function extractGraph(
  ...
  const prompt = `Episode summary: ${summary}\n\nKey learnings:\n${keyLearnings.map((l) => `- ${l}`).join('\n')}`;
